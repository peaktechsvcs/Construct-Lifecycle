...
**How to apply:** Validate the OpenAPI document first, reconcile any source/generated drift, then regenerate both the React client and Zod package together. If an operation combines path and query parameters, check for Orval's duplicate `*Params` export and keep the cleanup in the codegen postprocess rather than editing generated output by hand.