      ...
      <SceneLayout layout="stack" style={{ justifyContent: 'flex-start' }}>
        ...
        <motion.div className="paper-card" style={{ bottom: '8.5vmin', left: '14vmin', padding: '3.6vmin 3.3vmin 3.1vmin', position: 'absolute', width: '61vmin' }} initial={{ opacity: 0, x: -30, rotate: -3 }} animate={{ opacity: 1, x: 0, rotate: [-3, .7, 0] }} transition={{ duration: .95, delay: 1.25, ease: [0.16, 1, .3, 1] }}>
          <div style={{ alignItems: 'center', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ background: 'rgba(20,121,201,.12)', color: 'var(--color-primary)', fontFamily: 'var(--font-mono)', fontSize: '1.05vmin', letterSpacing: '.12em', padding: '1vmin 1.2vmin', textTransform: 'uppercase' }}>ready to send</div>
          </div>
          ...
          {['Walnut slab fronts', 'Taj Mahal surface', 'Brushed brass hardware'].map((line, index) => (
            <motion.div key={line} style={{ alignItems: 'center', borderBottom: index < 2 ? '1px solid rgba(6,43,85,.11)' : 'none', display: 'flex', justifyContent: 'space-between', padding: '1.1vmin 0' }} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .35, delay: 1.65 + index * .18 }}>
