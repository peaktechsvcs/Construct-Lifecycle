...
import { 
  ArrowRight, Pencil, MapPin, FileText, ClipboardList, Receipt, 
...
import { 
  ...
  stageLabels, currency, shortDate, fullDate
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 md:p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Badge tone={project.stage === 'billing' ? 'violet' : 'teal'}>{stageLabels[project.stage]}</Badge>
      </div>
      <div className="grid grid-cols-4 gap-2 md:grid-cols-7">
        {stages.map((stage, index) => (
          <div key={stage} className="relative">
            <div className={`mb-2 flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${index <= current ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground'}`}>
              {index < current ? <Check size={14} /> : index + 1}
            </div>
            <p className={`text-[10px] leading-4 ${index === current ? 'font-bold text-foreground' : 'text-muted-foreground'}`}>{stageLabels[stage]}</p>
            {index < stages.length - 1 && <span className={`absolute left-8 right-[-8px] top-4 h-px ${index < current ? 'bg-primary' : 'bg-border'}`} />}
          </div>
        ))}
      </div>
    </div>
    ...
    <section className="rounded-xl border border-border bg-card p-5 md:p-6">
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-secondary text-primary"><Icon size={16} /></span>
    ...
    <Modal title="Schedule a follow-up" onClose={onClose}>
      <form onSubmit={submit} className="space-y-5">
        <label className="block">
          ...
          <input data-testid="input-followup-date-detail" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} required className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-4 focus:ring-primary/20" />
        </label>
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-muted-foreground">What should happen next?</span>
          <textarea data-testid="textarea-followup-note-detail" value={note} onChange={(event) => setNote(event.target.value)} required rows={4} placeholder="e.g. Confirm revised countertop lead time with client" className="w-full resize-none rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none focus:ring-4 focus:ring-primary/20" />
        </label>
        <div className="flex justify-end gap-3">
          <Button data-testid="button-cancel-followup-detail" type="button" variant="ghost" onClick={onClose}>Cancel</Button>
          <Button data-testid="button-save-followup-detail" type="submit" disabled={create.isPending || !dueDate || !note}>{create.isPending ? 'Scheduling…' : 'Schedule follow-up'}</Button>
        </div>
      </form>
    </Modal>
  );
  ...
  return (
    <div className="animate-rise">
      <div className="mb-6 flex items-start gap-3">
        <Link href="/projects" data-testid="link-back-projects" className="mt-1 rounded-lg p-2 text-muted-foreground hover:bg-secondary hover:text-foreground">
          <ArrowRight size={17} className="rotate-180" />
        </Link>
        <div className="min-w-0 flex-1">
          <p className="mono text-[10px] uppercase tracking-[.15em] text-accent">{project.projectNumber} · {project.category}</p>
          ...
          <p className="mt-2 flex items-center gap-1.5 text-sm text-muted-foreground"><MapPin size={14} />{project.address || 'Address not added'} · {project.customerName}</p>
        </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
        <div className="space-y-5">
          <DetailCard icon={FileText} title="Proposal & bid" action={<Badge tone={project.bidOutcome === 'won' ? 'green' : project.bidOutcome === 'lost' ? 'red' : 'orange'}>{project.bidOutcome.replace('_', ' ')}</Badge>}>
          ...
          <DetailCard icon={ClipboardList} title="Contract & delivery" action={<Badge tone={project.contractStatus === 'active' ? 'teal' : 'neutral'}>{project.contractStatus}</Badge>}>
            <div className="grid gap-x-8 md:grid-cols-2">
              <DetailRow label="Contract value" value={currency.format(project.contractValue)} mono />
              <DetailRow label="Contract window" value={`${shortDate(project.contractStart)} — ${shortDate(project.contractEnd)}`} />
            ...
            <p className="mt-2 text-xs text-muted-foreground">{project.contractDetails || 'No contract notes added.'}</p>
          </DetailCard>
          
          <DetailCard icon={Receipt} title="Billing & collection" action={<Badge tone={project.billingStatus === 'paid' ? 'green' : 'violet'}>{project.billingStatus.replace('_', ' ')}</Badge>}>
          ...
          <DetailCard icon={PackageCheck} title="Closeout" action={<Badge tone={project.closeoutStatus === 'complete' ? 'green' : 'orange'}>{project.closeoutStatus.replace('_', ' ')}</Badge>}>
        ...
        <div className="space-y-5">
          <DetailCard icon={CalendarDays} title="Next follow-up" action={<Button data-testid="button-add-followup-detail" variant="outline" className="px-2.5 py-1.5 text-xs" onClick={() => setShowFollow(true)}><Plus size={13} /> Add</Button>}>
            <div className="rounded-lg border border-accent/25 bg-accent/8 p-4">
              <p className="mono text-[10px] uppercase tracking-[.12em] text-accent">{project.nextFollowUp ? shortDate(project.nextFollowUp) : 'Not scheduled'}</p>
              <p className="mt-2 text-sm font-semibold">{project.nextFollowUp ? 'Keep the next conversation warm.' : 'No follow-up is scheduled.'}</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Create a reminder for the next decision, delivery check-in, or referral conversation.</p>
            </div>
          </DetailCard>
