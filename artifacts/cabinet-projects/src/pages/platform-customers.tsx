...
import { InvitationDeliveryOutcome, PlatformCustomerStatus, UpdatePlatformCustomerInputStatus, useCreatePlatformCustomer, useListPlatformCustomers, useUpdatePlatformCustomer, getListPlatformCustomersQueryKey } from '@workspace/api-client-react';
...
export function PlatformCustomers() {
  ...
  const [name, setName] = useState(''); const [slug, setSlug] = useState(''); const [ownerEmail, setOwnerEmail] = useState(''); const [link, setLink] = useState<string | null>(null); const [delivery, setDelivery] = useState<InvitationDeliveryOutcome | null>(null); const [copied, setCopied] = useState(false);
  ...
  return <div className="animate-rise"><PageTitle eyebrow="Platform" title="Customers" description="Onboard and control customer workspaces." /><div className="grid gap-6 lg:grid-cols-[1fr_360px]"><section className="rounded-xl border border-border bg-card p-5"><h2 className="mb-4 text-base font-bold">Customer workspaces</h2>{(customers.data ?? []).length === 0 ? <EmptyState icon={Building2} title="No customers yet" text="Create a customer workspace to get started." /> : <div className="space-y-3">{(customers.data ?? []).map((customer) => <div key={customer.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-border p-4"><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{customer.name}</p><p className="mono truncate text-[10px] text-muted-foreground">{customer.slug} · {customer.memberCount} members · {customer.pendingInvitationCount} pending</p></div><Badge tone={customer.status === PlatformCustomerStatus.active ? 'green' : 'red'}>{customer.status}</Badge><Button variant="outline" disabled={update.isPending} onClick={() => update.mutate({ tenantId: customer.id, data: { status: customer.status === 'active' ? UpdatePlatformCustomerInputStatus.suspended : UpdatePlatformCustomerInputStatus.active } }, { onSuccess: refresh })}>{customer.status === 'active' ? <><Pause size={14} /> Suspend</> : <><Play size={14} /> Reactivate</>}</Button></div>)}</div>}</section><section className="rounded-xl border border-border bg-card p-5"><div className="mb-5 flex items-center gap-3 border-b border-border pb-4"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-secondary text-primary"><Plus size={16} /></span><h2 className="text-base font-bold">Create customer</h2></div><form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setLink(null); setDelivery(null); setCopied(false); create.mutate({ data: { name, slug, ownerEmail: ownerEmail || null, businessTypes: ['general-contractor'] } }, { onSuccess: (result) => { setName(''); setSlug(''); setOwnerEmail(''); setDelivery(result.invitationDelivery); refresh(); if (result.invitationToken) setLink(`${window.location.origin}${import.meta.env.BASE_URL}accept-invitation/${result.invitationToken}`); } }); }}><label className="block"><span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Customer name</span><input required minLength={2} value={name} onChange={(e) => setName(e.target.value)} className={inputClass} /></label><label className="block"><span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Slug</span><input required pattern="[a-z0-9][a-z0-9-]{2,62}" value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase())} className={inputClass} placeholder="acme-builders" /></label><label className="block"><span className="mb-1.5 block text-xs font-semibold text-muted-foreground">Owner email <span className="font-normal text-muted-foreground">(optional)</span></span><input type="email" value={ownerEmail} onChange={(e) => setOwnerEmail(e.target.value)} className={inputClass} /></label><Button type="submit" disabled={create.isPending}><Plus size={15} /> {create.isPending ? 'Creating…' : 'Create customer'}</Button>{create.isError && <p role="alert" className="text-xs text-destructive">Customer could not be created. Try again.</p>}</form>{link && <div className="mt-5 rounded-lg border border-primary/20 bg-primary/5 p-4"><p role="status" className={`text-xs font-semibold ${delivery === 'sent' ? 'text-status-success' : delivery === 'failed' ? 'text-status-danger' : 'text-status-warning'}`}>{delivery === 'sent' && 'Owner invitation email sent.'}{delivery === 'failed' && 'Email delivery failed; the owner invitation is still pending.'}{delivery === 'not_configured' && 'Email delivery is not configured; share the recovery link below.'}</p><p className="mt-2 text-xs text-muted-foreground">This one-time recovery link is shown only now and expires in seven days.</p><div className="mt-2 flex gap-2"><input readOnly value={link} aria-label="Owner invitation link" className={`${inputClass} text-xs`} /><Button variant="outline" onClick={() => { navigator.clipboard.writeText(link); setCopied(true); }}><Copy size={14} /> {copied ? 'Copied' : 'Copy'}</Button></div></div>}</section></div></div>;
}