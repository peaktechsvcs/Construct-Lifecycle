...
export function WorkflowsAdmin() {
  ...
  const save = () => {
    ...
    ensureDraft(() => updateDraft.mutate({
      data: { name, description: description || null, states, statuses, transitions },
    }, {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getGetWorkflowConfigQueryKey() });
    ...
    <div className="animate-rise">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
        ...
        <section className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="mt-6 border-t border-border pt-5"><h3 className="text-sm font-bold">Statuses</h3><p className="mt-1 text-xs text-muted-foreground">Status is separate from lifecycle state.</p><div className="mt-3 space-y-2">{statuses.map((status) => <div key={status.stableKey} className="flex items-center gap-3 rounded-lg border border-border p-3"><Check size={14} className="text-primary" /><input value={status.displayName} onChange={(event) => setStatuses((current) => current.map((item) => item.stableKey === status.stableKey ? { ...item, displayName: event.target.value } : item))} className="min-w-0 flex-1 rounded-md border border-transparent bg-transparent px-2 py-1 text-sm font-semibold focus:border-input focus:bg-background" /><span className="mono text-[10px] text-muted-foreground">{status.stableKey}</span></div>)}</div><div className="mt-6 border-t border-border pt-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-bold">Allowed transitions</h3><p className="mt-1 text-xs text-muted-foreground">Server-side rules control which roles can move projects between states.</p></div><Button variant="outline" onClick={addTransition}><Plus size={14} /> Add</Button></div><div className="mt-3 space-y-3">{transitions.map((transition, index) => <div key={`${transition.fromStateKey}-${transition.toStateKey}-${index}`} className="rounded-lg border border-border p-3"><div className="grid gap-2 sm:grid-cols-2"><select aria-label="From state" value={transition.fromStateKey} onChange={(event) => setTransitions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, fromStateKey: event.target.value } : item))} className="rounded-md border border-input bg-background px-2 py-2 text-xs">{states.map((state) => <option key={state.stableKey} value={state.stableKey}>{state.displayName}</option>)}</select><select aria-label="To state" value={transition.toStateKey} onChange={(event) => setTransitions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, toStateKey: event.target.value } : item))} className="rounded-md border border-input bg-background px-2 py-2 text-xs">{states.map((state) => <option key={state.stableKey} value={state.stableKey}>{state.displayName}</option>)}</select></div><div className="mt-2 grid gap-2 sm:grid-cols-2"><input aria-label="Allowed roles" value={(transition.allowedRoles ?? []).join(', ')} onChange={(event) => setTransitions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, allowedRoles: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) } : item))} placeholder="Allowed roles: owner, admin" className="rounded-md border border-input bg-background px-2 py-2 text-xs" /><input aria-label="Required fields" value={(transition.requiredFields ?? []).join(', ')} onChange={(event) => setTransitions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, requiredFields: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) } : item))} placeholder="Required fields: contractStart" className="rounded-md border border-input bg-background px-2 py-2 text-xs" /></div><label className="mt-2 flex items-center gap-2 text-xs"><input type="checkbox" checked={transition.active !== false} onChange={(event) => setTransitions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, active: event.target.checked } : item))} /> Active</label></div>)}</div></div></div>
        </section>
      </div>
    </div>
  );
