...
function MilestoneEditor({ draft, setDraft, onSave, pending, editing }: { draft: Draft; setDraft: (draft: Draft) => void; onSave: () => void; pending: boolean; editing: boolean }) {
  const field = (label: string, key: keyof Draft, type = 'text') => <label className="block"><span className="mb-1 block text-[10px] font-semibold text-muted-foreground">{label}</span><Input type={type} value={draft[key]} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} className="h-9 bg-background text-xs" /></label>;
  ...
  </div><Button className="mt-4 px-3 py-2 text-xs" disabled={pending || !draft.itemNumber.trim() || !draft.name.trim()} onClick={onSave}>{pending ? 'Saving…' : editing ? 'Save milestone' : 'Add milestone'}</Button></div>;
}
...
function MilestoneProject({ project }: { project: Project }) {
  ...
  return <article className="rounded-xl border border-border bg-card p-4 md:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="mono text-[10px] text-accent">{project.projectNumber}</p><Link href={`/projects/${project.id}`} className="text-base font-bold hover:text-primary hover:underline">{project.projectName}</Link><p className="text-xs text-muted-foreground">{project.customerName}</p></div><div className="flex gap-2"><Button variant="outline" className="px-3 py-1.5 text-xs" onClick={() => open()}><Plus size={13} /> Add milestone</Button><Link href={`/projects/${project.id}`} className="rounded-md px-2 py-1.5 text-xs font-semibold text-primary hover:bg-secondary">Project detail</Link></div></div>
