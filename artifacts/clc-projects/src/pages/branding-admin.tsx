...
export function BrandingAdmin() {
  ...
  if (brandingQuery.isLoading) return <><PageTitle eyebrow="Settings / Branding" title="Customer Branding" description="Customize Construct Lifecycle to match your organization's visual identity." /><LoadingPanel lines={6} /></>;
  ...
  const colorIssues = getBrandingColorIssues(form);
  const invalidColorFields = getInvalidBrandingColorFields(form);
  const hasColorIssues = Object.keys(colorIssues).length > 0;
  const hasInvalidColorFields = invalidColorFields.length > 0;
  const contrastFailures = contrastChecks.filter((check) => check.ratio < 4.5);
  const colorField = (key: BrandingColorKey, label: string) => {
    const inputId = `branding-${key}`;
    const issues = colorIssues[key] ?? [];
    return (
      <label className="block" key={key}>
        ...
        <div className="flex gap-2">
          <input
            ...
            value={normalizeHexColor(form[key]) || previewColors[key]}
          ...
          <input
            ...
            aria-describedby={issues.length > 0 ? `${inputId}-guidance` : undefined}
            onChange={(e) => handleChange(key, e.target.value)}
            className={`flex-1 rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:ring-4 focus:ring-primary/20 ${issues.length > 0 ? 'border-destructive/60' : 'border-input'}`}
          />
        </div>
        {issues.length > 0 && (
          <div id={`${inputId}-guidance`} className="mt-1.5 space-y-1 text-xs text-destructive">
  ...
  return (
    <div className="animate-rise">
        <PageTitle eyebrow="Settings / Branding" title="Customer Branding" description="Customize Construct Lifecycle to match your organization's visual identity." action={
        ...
        <div className="space-y-6">
          <section className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-4">
              ...
              {colorField('foregroundColor', 'Text Color (Foreground)')}
            ...
            {hasInvalidColorFields && (
              <div role="status" className="mt-5 flex items-start gap-2 rounded-lg border border-status-warning/30 bg-status-warning/10 p-3 text-xs text-status-warning">
                ...
                <span>Draft autosave is paused until the highlighted color values are corrected. The preview is using safe fallback colors while you edit.</span>
              </div>
            )}
            {!hasInvalidColorFields && hasColorIssues && (
              <div role="status" className="mt-5 flex items-start gap-2 rounded-lg border border-status-warning/30 bg-status-warning/10 p-3 text-xs text-status-warning">
                <AlertTriangle size={14} className="mt-0.5 shrink-0" />
            ...
            <div className="mt-6 border-t border-border pt-5">
              ...
              {(() => {
                const passes = !hasColorIssues && contrastFailures.length === 0;
