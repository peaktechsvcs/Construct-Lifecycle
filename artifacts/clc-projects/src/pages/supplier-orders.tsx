...
function marginPercent(grossMargin: number, totalSell: number) {
  if (totalSell <= 0) return '—';
  return `${Math.round((grossMargin / totalSell) * 100)}%`;
}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
          ...
          <Section eyebrow="Conversion" title="Quote detail">
             {unavailableQuoteLink ? <div data-testid="supplier-quote-unavailable"><EmptyState icon={Receipt} title="Quote unavailable" text="This quote link is invalid or no longer available. Choose a quote from the list instead." /></div> : !selectedQuoteId ? <EmptyState icon={Receipt} title="Choose a quote" text="Select a quote to review margin and convert accepted supplier pricing into an order." /> : selectedQuote.isError ? <SupplierDataError testId="supplier-quote-detail-error" title="Supplier quote detail unavailable" text="The selected quote could not be loaded. Retry to review its lines and totals." onRetry={() => { void selectedQuote.refetch(); }} /> : selectedQuote.isLoading ? <LoadingPanel lines={4} /> : selectedQuote.data ? <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Metric label="Sell" value={money(selectedQuote.data.totalSell)} /><Metric label="Cost" value={money(selectedQuote.data.totalCost)} /><Metric label="Margin" value={money(selectedQuote.data.grossMargin)} /><Metric label="Margin %" value={marginPercent(selectedQuote.data.grossMargin, selectedQuote.data.totalSell)} /></div>
...
function QuoteTable({ quotes, selectedQuoteId, onSelect }: { quotes: SupplierQuote[]; selectedQuoteId?: number; onSelect: (id: number) => void }) {
  return <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-border text-xs text-muted-foreground"><tr><th className="pb-2 font-semibold">Quote</th><th className="pb-2 font-semibold">Customer</th><th className="pb-2 font-semibold">Sell</th><th className="pb-2 font-semibold">Margin</th><th className="pb-2 font-semibold">Margin %</th><th className="pb-2 font-semibold">Status</th></tr></thead><tbody className="divide-y divide-border">{quotes.map((quote) => <tr key={quote.id} data-testid={`row-supplier-quote-${quote.id}`} tabIndex={0} role="button" className={`cursor-pointer transition-colors hover:bg-secondary/50 ${selectedQuoteId === quote.id ? 'bg-primary/5' : ''}`} onClick={() => onSelect(quote.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(quote.id); }}><td className="py-3 font-bold">{quote.quoteNumber}</td><td className="py-3">{quote.customerName}</td><td className="py-3">{money(quote.totalSell)}</td><td className="py-3">{money(quote.grossMargin)}</td><td className="py-3 font-semibold" data-testid={`text-supplier-quote-margin-percent-${quote.id}`}>{marginPercent(quote.grossMargin, quote.totalSell)}</td><td className="py-3"><Badge tone={statusTone(quote.status)}>{labelStatus(quote.status)}</Badge></td></tr>)}</tbody></table></div>;
}

function OrderTable({ orders, selectedOrderId, onSelect }: { orders: SupplierOrder[]; selectedOrderId?: number; onSelect: (id: number) => void }) {
  return <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="border-b border-border text-xs text-muted-foreground"><tr><th className="pb-2 font-semibold">Order</th><th className="pb-2 font-semibold">Customer</th><th className="pb-2 font-semibold">Promised</th><th className="pb-2 font-semibold">Sell</th><th className="pb-2 font-semibold">Margin</th><th className="pb-2 font-semibold">Margin %</th><th className="pb-2 font-semibold">Status</th></tr></thead><tbody className="divide-y divide-border">{orders.map((order) => <tr key={order.id} data-testid={`row-supplier-order-${order.id}`} tabIndex={0} role="button" className={`cursor-pointer transition-colors hover:bg-secondary/50 ${selectedOrderId === order.id ? 'bg-primary/5' : ''}`} onClick={() => onSelect(order.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(order.id); }}><td className="py-3 font-bold">{order.orderNumber}</td><td className="py-3">{order.customerName}</td><td className="py-3">{shortDate(order.promisedDate)}</td><td className="py-3">{money(order.totalSell)}</td><td className="py-3">{money(order.grossMargin)}</td><td className="py-3 font-semibold" data-testid={`text-supplier-order-margin-percent-${order.id}`}>{marginPercent(order.grossMargin, order.totalSell)}</td><td className="py-3"><Badge tone={statusTone(order.orderStatus)}>{labelStatus(order.orderStatus)}</Badge></td></tr>)}</tbody></table></div>;
}